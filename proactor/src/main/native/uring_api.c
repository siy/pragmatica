...
#include "include/org_pfj_io_uring_Uring.h"
...
JNIEXPORT jint JNICALL Java_org_pfj_io_uring_Uring_init(JNIEnv *env, jclass clazz, jint num_entries, jlong base_address, jint flags) {
    return (jint) io_uring_queue_init((unsigned) num_entries, RING_PTR, (unsigned) flags);
}

JNIEXPORT void JNICALL Java_org_pfj_io_uring_Uring_close(JNIEnv *env, jclass clazz, jlong base_address) {
    io_uring_queue_exit(RING_PTR);
}

JNIEXPORT jint JNICALL Java_org_pfj_io_uring_Uring_peekCQ(JNIEnv *env, jclass clazz, jlong base_address, jlong completions_address, jlong count) {
    return (jint) io_uring_peek_batch_cqe(RING_PTR, CQE_BATCH_PTR, COUNT);
}

JNIEXPORT void JNICALL Java_org_pfj_io_uring_Uring_advanceCQ(JNIEnv *env, jclass clazz, jlong base_address, jlong count) {
    io_uring_cq_advance(RING_PTR, COUNT);
}

JNIEXPORT jint JNICALL Java_org_pfj_io_uring_Uring_readyCQ(JNIEnv *env, jclass clazz, jlong base_address) {
    return (jint) io_uring_cq_ready(RING_PTR);
}

JNIEXPORT jlong JNICALL Java_org_pfj_io_uring_Uring_spaceLeft(JNIEnv *env, jclass clazz, jlong base_address) {
    return (jlong) io_uring_sq_space_left(RING_PTR);
}

JNIEXPORT jlong JNICALL Java_org_pfj_io_uring_Uring_nextSQEntry(JNIEnv *env, jclass clazz, jlong base_address) {
...
JNIEXPORT jint JNICALL Java_org_pfj_io_uring_Uring_peekSQEntries(JNIEnv *env, jclass clazz, jlong base_address, jlong submissions_address, jlong space) {
    if (io_uring_sq_space_left(RING_PTR) < 1) {
        io_uring_submit_and_wait(RING_PTR, 1);
    }
...
JNIEXPORT jlong JNICALL Java_org_pfj_io_uring_Uring_submitAndWait(JNIEnv *env, jclass clazz, jlong base_address, jint count) {
...
JNIEXPORT jint JNICALL Java_org_pfj_io_uring_Uring_socket(JNIEnv *env, jclass clazz, jint domain, jint socket_type, jint socket_options) {
...
JNIEXPORT jint JNICALL Java_org_pfj_io_uring_Uring_prepareForListen(JNIEnv *env, jclass clazz, jint sock, jlong address, jint len, jint queue_depth) {
