...
public class TcpClient<T extends InetAddress> implements Client {
    ...
    public <R> Promise<R> access(ClientProtocol<R> protocol) {
        return Promise.promise(promise -> createSocket(promise, protocol));
    }

    private <R> void createSocket(Promise<R> promise, ClientProtocol<R> protocol) {
        proactor().socket(result -> result.onFailure(promise::failure)
                                          .onSuccess(socket -> doConnect(socket, promise, protocol))
    ...
    private <R> void closeSocket(Promise<R> promise, FileDescriptor socket) {
        promise.onResultDo(() -> proactor().close(Unit::unit, socket));
    }

    private <R> void doConnect(FileDescriptor socket, Promise<R> promise, ClientProtocol<R> protocol) {
        proactor().connect(result -> handleConnection(result, protocol, socket, promise),
    ...
    private <R> void handleConnection(Result<FileDescriptor> result,
        ...
        result.onFailure(promise::failure)
              .onSuccessDo(() -> protocol.process(connection(socket, config.address()))
