...
public class ClientConnector<T extends InetAddress> {
    ...
    public Promise<ClientConnectionContext<T>> connect() {
        return PromiseIO.socket(selectSocketType())
                        .flatMap(fd -> PromiseIO.connect(fd, address))
                        .map(fd -> new ClientConnectionContext<>(address, fd));
    }

    private SocketType selectSocketType() {
        return type == ConnectorType.UDP ? SocketType.DGRAM : SocketType.STREAM;
    }
}
