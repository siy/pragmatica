...
public class RequestTest {
    ...
    void makeRequest() {
        ...
        try (var query = OffHeapSlice.fixedSize(4096);
            ...
            var result = PromiseIO.socket(AddressFamily.INET, SocketType.DGRAM, SocketFlag.closeOnExec(), SocketOption.reuseAll())
                                  .onSuccess(socket::set)
                                  .flatMap(() -> PromiseIO.connect(socket.get(), socketAddress))
                                  .flatMap(() -> PromiseIO.write(socket.get(), query))
                                  .flatMap(() -> PromiseIO.read(socket.get(), buffer))
                                  .join();
